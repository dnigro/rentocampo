...
export default async function LandingMapa() {
  ...
  const { data: campos } = await supabase.from("campos").select("id, titulo, provincia, localidad, lat, lng, hectareas, aptitud, precio_ha, moneda").eq("status", "activo").not("lat", "is", null).not("lng", "is", null);
  return (
