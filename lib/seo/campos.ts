...
> = {
  agricolas: { value: "agricola", label: "Campos agrícolas", singular: "agrícola" },
  ganaderos: { value: "ganadera", label: "Campos ganaderos", singular: "ganadero" },
  mixtos: { value: "mixta", label: "Campos mixtos", singular: "mixto" },
  forestales: { value: "forestal", label: "Campos forestales", singular: "forestal" },
};
...
export function campoDescription(campo: {
  ...
  const aptitud =
    Object.values(APTITUD_SEO).find((item) => item.value === campo.aptitud)
      ?.singular ?? campo.aptitud ?? "rural";
  const ubicacion = [campo.localidad, campo.provincia].filter(Boolean).join(", ");
